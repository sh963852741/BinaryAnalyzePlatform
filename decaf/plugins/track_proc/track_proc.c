...
static target_ulong expected_proc_cr3  = 0 ;
static char * expected_proc_name = DECAF_NULL_HANDLE; // i.e., expected_mod_name
static target_ulong expected_proc_pc_start = 0;
static target_ulong expected_proc_pc_end = 0;
...
static int load_export_symbols(const char *expected_mod_name)
{
    ...
    while(!feof(fp)){
        ...
        printf("mod_name=\"%s\" elf_name=\"%s\" base_addr=\"%x\" func_addr= \"%x\", size= \"%d\" \n",mod_name, func_name, base_addr, func_addr, size);    
        ...
        printf("func_name=\"%s\" start_addr=\"%x\" end_addr= \"%x\", size= \"%d\" \n",func_name, func_addr, func_addr+size, size);    
		func_blk_cnt++;
		found_mod = true;
...
void do_block_start_cb(DECAF_Callback_Params *param)
{
        ...
        if(!export_symbols_loaded)
	{
        ...
        if(expected_proc_cr3 && (cr3 == expected_proc_cr3))
  	{
            if(export_symbols_loaded)
            {
		...
		{ 
                    ;//printf("get func fast fail, pc %x------------ \n", param->bb.tb->pc);
		}
            }
	    else
	...
	if (0 == funcmap_get_name_c(param->bb.tb->pc, cr3, &mod_name, &func_name))
	{
	    ...
	    {
	        printf("---- START: %s %s \n", mod_name, func_name);
		expected_proc_cr3 = cr3;
		set_expected_cr3(cr3);
		if(0 == funcmap_get_pc_range_by_pc(param->bb.tb->pc, cr3, &expected_proc_pc_start, &expected_proc_pc_end))
                    set_expected_pc_range(expected_proc_pc_start, expected_proc_pc_end); // set the start/end address of vma of mod
	    }
#endif
...
void load_module_notify(VMI_Callback_Params * params)
{
    ...
    // load the extraced symbol table of expeced module
    if(!export_symbols_loaded)
    {
        int ret = load_export_symbols(expected_proc_name);
        if(ret == -1)
        {
	    printf("impossible ??? \n");
	    char tmp_mod_name[NAME_LEN];
	    char tmp_func_name[NAME_LEN];
            // exported_symbols file is missing, so force to call VMI_extract_symbols() which generates exported_symbols
            //printf("load_export_symbols fail \n");
            //funcmap_get_name_c(param->bb.tb->pc, cr3, &tmp_mod_name, &tmp_func_name);
        }
        else
            printf("load_export_symbols success \n");
    }
    ...
    expected_proc_cr3 = cr3;
    set_expected_cr3(cr3);
    if(0 == funcmap_get_pc_range_by_name(mod_name, cr3, &expected_proc_pc_start, &expected_proc_pc_end))
        set_expected_pc_range(expected_proc_pc_start, expected_proc_pc_end); // set the start/end address of vma of mod
    else
        printf("cannot get pc_range \n");
}
...
void track_proc_cleanup()
{
	...
	expected_proc_pc_start = 0;
	expected_proc_pc_end = 0;
        expected_proc_cr3 = 0;
	...
	//TODO: mod->extracted = false
	remove("exported_symbols.log");
}
