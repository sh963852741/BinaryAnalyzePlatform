...
int read_elf_info(const char * mod_name, target_ulong start_addr, unsigned int inode_number) {
	...
	bool expected_proc = false;
        char expected_mod_name[128];
        memset(expected_mod_name, 0x00, 128);
        int get_mod_ret = get_expected_mod_name(expected_mod_name);
	if(!get_mod_ret && (strcmp(mod_name, expected_mod_name) == 0)) //"hello" / "helloworld"
		expected_proc = true;
        else
		printf("not expected mod name, %d, %s, %s \n", get_mod_ret, mod_name, expected_mod_name);

	if(expected_proc)
		printf("    -- read_elf_info, mod_name is %s, start_addr is %lu \n", mod_name, start_addr);
	...
	if(expected_proc)
		printf("    -- read_elf_info, sec_num is %d \n", sec_num);
	for ( int i = 0; i < sec_num; ++i ) {
		...
		if ( psec->get_type() == SHT_DYNSYM || psec->get_type() == SHT_SYMTAB) {
			...
			if(expected_proc)
				printf("    -- read_elf_info, DYNSYM or SYMTAB \n");
			const symbol_section_accessor symbols( reader, psec );
			if(expected_proc)
				printf("    -- read_elf_info, symbol_cnt is %d \n", symbols.get_symbols_num());
			for ( unsigned int j = 0; j < symbols.get_symbols_num(); ++j ) {
				...
				if(type == STT_FUNC ) {
					register_symbol(mod_name, name.c_str(), (value-elf_entry), inode_number);
			   	    if(expected_proc)
				    {		
					printf("---- funcname: %s, addr: %lu, size: %d \n", name.c_str(), (unsigned long)(value-elf_entry), int(size));
					strcpy(tmp_func_name, name.c_str());
					tmp_size = int(size);
					//fprintf(fp, "mod_name=\"%s\" elf_name=\"%s\" base_addr=\"%x\" func_addr= \"%x\", size= \"%d\" \n",mod_name, tmp_func_name,elf_entry, value, int(size));
					//fprintf(fp, "mod_name=\"%s\" elf_name=\"%s\" base_addr=\"%x\" func_addr= \"%x\", size= \"%d\" \n",mod_name, tmp_func_name,elf_entry, value, int(size));
					fprintf(fp, "mod_name=%s elf_name=%s base_addr=%x func_addr=%x, size=%d \n",mod_name, tmp_func_name,elf_entry, value, tmp_size);
					fflush(fp);
				    }
 				}
				else
				{
					;
					//if(expected_proc)
					//	printf("    -- read_elf_info, not STT_FUNC \n");
				}
			}
		}
		else
		{
			;
			//if(expected_proc)
			//	printf("    -- read_elf_info, other type \n");
		}
	}
