...
{
    printf("new_kmod_callback was called!!!");
...
{
	//printf("traverse task struct add \n");
    uint32_t task_pid = 0;
...
void traverse_mmap(CPUState *env, void *opaque)
{
    ...
    // Added by Lei
    bool expected_proc = false;
    // printf("traverse_mmap \n");
    char expected_proc_name[128];
    memset(expected_proc_name, 0x00, 128);
    int get_mod_ret = get_expected_mod_name(expected_proc_name);
    if(!get_mod_ret && (strcmp(proc->name, expected_proc_name) == 0))
        expected_proc = true;
    //else
    //	printf("not expected ");
    ...
    {
        if(expected_proc)
            printf("mm_mmap is 0, exit traverse_mmap \n");
        return;
    }
    ...
    while(true)
    {
        ...
        {
	    if(expected_proc)
                printf("    ++ cannot get vma_vm_start, next vma \n");
            goto next;
	}
        else
        {
	    if(expected_proc)
                printf("    ++ get vma_vm_start, %lu \n", vma_vm_start);
	}
        // read end of curr vma
        if (DECAF_read_mem(env, vma_curr + OFFSET_PROFILE.vma_vm_end, sizeof(target_ptr), &vma_vm_end) < 0)
        {
            if(expected_proc)
                printf("    ++ cannot get vma_vm_end, next vma \n");
            goto next;
        }
	else
	{
	    if(expected_proc)
                printf("    ++ get vma_vm_end, %lu \n", vma_vm_end);
	}
	...
	if(expected_proc)
        ...
        if (!mod)
        {
            ...
            mod->name[31] = '\0';
	    if(expected_proc)
		printf("    ++ new and add a module, %s \n", mod->name);
            mod->size = vma_vm_end - vma_vm_start;
	    // Added by Lei
	    mod->pc_start = vma_vm_start;
 	    mod->pc_end = vma_vm_end;
            // End add
...
{
	printf("Linux_tlb_call_back was called. \n");
    ...
    {
	    printf("Caution: VMI_LOADMODULE_CB and VMI_REMOVEMODULE_CB were not registed! (at linux_vmi_new.cpp)\n");
        return;
    }

    // printf("tlb_call_back \n");
    ...
    {
        printf("start traverse mmap \n");
        traverse_mmap(ourenv, proc);
    }
    else
    {
        printf("Caution: proc is null or modules have not been extracted! (at linux_vmi_new.cpp)\n");
    }
    //if(!proc)
//	printf("proc is null\n");
  //  else if(proc->modules_extracted)
    //    printf("modules has extracted");
}
...
void linux_vmi_init()
{
	printf("linux vmi init\n");
    DECAF_registerOptimizedBlockBeginCallback(&new_proc_callback, NULL, OFFSET_PROFILE.proc_exec_connector, OCB_CONST);
    // TODO, comment it, added by Lei
	//DECAF_registerOptimizedBlockEndCallback(NULL, NULL, OFFSET_PROFILE.proc_exec_connector, OCB_CONST);
	...
	kernel_proc->pid = 0;
        printf("linux_vmi_init\n");
	VMI_create_process(kernel_proc);
        printf("linux_vmi_over\n");
}
