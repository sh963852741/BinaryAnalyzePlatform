...
{
	//printf("VMI_find_module_by_base \n");
...
{
	//printf("VMI_find_module_by_name \n");
	unordered_map < uint32_t, process * >::iterator iter_p = process_map.find(pgd);
	if (iter_p == process_map.end())
		return NULL;
...
int VMI_add_module(module *mod, const char *key){
	...
	//if(strcmp(mod->name, "hello") == 0 || strcmp(mod->name, "helloworld") == 0)
	//	printf("++++ VMI_add_module %s \n", mod->name);
...
//		printf("+++ VMI_create_process %s \n", proc->name);
	//if(strcmp(proc->name, "hello") == 0 || strcmp(proc->name, "helloworld") == 0)
	//	printf("+++ VMI_create_process %s \n", proc->name);
...
{
	printf("VMI_insert_module %s", mod->name);
	VMI_Callback_Params params;
	params.lm.pid = pid;
	params.lm.base = base;
	...
	if (iter == process_pid_map.end()) //pid not found
	{
		printf("VMI_insert_module %s failed. process id was not found", mod->name);
		return -1;
	}
	...
	// Added by Lei 
	char expected_mod_name[256];
	memset(expected_mod_name, 0x00, 256);
	int ret = get_expected_mod_name(expected_mod_name);
	if(!ret)
	{
		printf("The expected module name id %s \n", expected_mod_name);
            //if(strcmp(mod->name, "hello") == 0 || strcmp(mod->name, "helloworld") == 0)
		if(strcmp(mod->name, expected_mod_name) == 0)
		{
...
int VMI_extract_symbols(module *mod, target_ulong base)
{
	...
	{
		printf("Caution: inode_number is 0. (at vmi.cpp, at VMI_extract_symbols)\n");
		return 0;
	}
