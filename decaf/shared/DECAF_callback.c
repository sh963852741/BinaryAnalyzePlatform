...
// Added by Lei
int track_flag;
target_ulong expected_cr3 = 0; // used to filter dumped tcgs upon translation
//static char *expected_mod_name = NULL;
static char expected_mod_name[256];
target_ulong expected_pc_start = 0; // used to denote the start address of vma area of expected proc
target_ulong expected_pc_end = 0; // used to denote the end address of vma ares of expected proc
FILE * tcg_log_file = NULL;

static int DEBUG = 1;

void DECAF_log_info(char * info)
{
	//return; // TODO: too slow for ubuntu
	int fd;
	fd = open("/tmp/decaf.log", O_CREAT | O_RDWR | O_APPEND, 0777);
	if(fd < 0)
	{
...
void set_expected_cr3(target_ulong cr3)
{
    if(DEBUG)
        printf("set_expected_cr3 %lu \n", (unsigned long)cr3);
    expected_cr3 = cr3;
}

target_ulong get_expected_cr3()
{
    if(DEBUG && expected_cr3 != 0)
        printf("get_expected_cr3 %lu \n", (unsigned long)expected_cr3);
    return expected_cr3;
}

int get_expected_mod_name(char * mod_name)
{
    //printf("get expected_mod_name \n");
    if(strlen(expected_mod_name) == 0)
    {
        printf("get_expected_mod_name NULL \n");
        return -1;
    }
    //else
    //    printf("get_expected_mod_name %s \n", expected_mod_name);
    strcpy(mod_name, expected_mod_name);
    return 0;
}

void set_expected_mod_name(char * mod_name)
{
    //fprintf(expected_mod_name, "%s", mod_name);
    memset(expected_mod_name, 0x00, 256);
    strcpy(expected_mod_name, mod_name);
    if(DEBUG)
        printf("set_expected_mod_name2 %s \n", mod_name);
}


FILE * get_tcg_log_file()
{
    if(!tcg_log_file)
        printf("get_tcg_log_file NULL \n");
    else
        printf("get_tcg_log_file not NULL \n");
    return tcg_log_file;
}

void set_tcg_log_file(bool set_file)
{
    if(set_file) // crate log_file
    {
       if(tcg_log_file)
           tcg_log_file = NULL;
       printf("set tcg log file, start \n");
       char log_name[256];
       #if defined(TARGET_I386) || defined(TARGET_X86_64)
           sprintf(log_name, "/tmp/tcg_log_x86.log");
       #elif defined(TARGET_MIPS) 
           sprintf(log_name, "/tmp/tcg_log_mips.log");
       #elif defined(TARGET_ARM) 
           sprintf(log_name, "/tmp/tcg_log_arm.log");
       #else // Trigger crash?
           printf("ERROR: unsupported arch \n");
       #endif
       printf("set tcg log file, end \n");
       tcg_log_file = fopen(log_name, "w+");
    }
    else // clear log_file
    {
        if(tcg_log_file)
...
void set_expected_pc_range(target_ulong pc_start, target_ulong pc_end)
{
    printf("set pc range, start: %lu, end: %lu \n", (unsigned long)pc_start, (unsigned long)pc_end);
    expected_pc_start = pc_start; 
    expected_pc_end = pc_end;
}

void get_expected_pc_range(target_ulong * pc_start, target_ulong * pc_end)
{
    *pc_start = expected_pc_start;
    *pc_end = expected_pc_end;
}
// End
...
DECAF_Handle DECAF_registerOptimizedBlockBeginCallback(
    ...
    OCB_t type)
{
	//printf("registerOptimizedBlockBeginCallback \n"); // Lei
...
// Added by Lei
#if 1
	if (1 && record_bb) //track_flag == 1)
	{
		char info[512];
		memset(info, 0x00, 512);
		//if((unsigned long)params.bb.tb->pc < 136000000) //TODO, only log the code in text segment
		if((unsigned long)params.bb.tb->pc < 196000000) //TODO, only log the code in text segment
		{
			sprintf(info, "begincall helper, pc: %lu, size: %d, icount: %u, pc2: %lu, size2: %d, icount2: %u\n", (unsigned long)tb->pc, tb->size,tb->icount, (unsigned long)params.bb.tb->pc, params.bb.tb->size, params.bb.tb->icount);
			//printf("block_start, pc: %lu, size: %d, icount: %u, pc2: %lu, size2: %d, icount2: %u\n", (unsigned long)tb->pc, tb->size,tb->icount, (unsigned long)params.bb.tb->pc, params.bb.tb->size, params.bb.tb->icount);
			//fflush(stdout);
			//DECAF_log_info(info);
		}
	}
...
PUSH_ALL()
  ...
  LIST_FOREACH_SAFE(cb_struct, &callback_list_heads[DECAF_BLOCK_BEGIN_CB], link, tmp){
    //printf("helper_DECAF_invoke_block_begin_callback, not threadsafe \n");
	//if(track_flag == 1)
	//{
		//DECAF_log_info("***** haha  \n");
	//	i++;
...
POP_ALL()
	...
	if(0) //(track_flag == 1)
	{
		...
		sprintf(info, "call_back func cnt %d \n", i);
		//DECAF_log_info(info);
	}
#endif
}
...
void helper_DECAF_invoke_block_end_callback(CPUState* env, TranslationBlock* tb, gva_t from)
{
  ...
  params.be.cur_pc = from;
    ...
    {
	record_bb = true;
 	//printf("block_end, pc: %lu, size: %d, icount: %u, pc2: %lu, size2: %d, icount2: %u\n", (unsigned long)tb->pc, tb->size,tb->icount, (unsigned long)params.bb.tb->pc, params.bb.tb->size, params.bb.tb->icount);
	//fflush(stdout);
    }
PUSH_ALL()
