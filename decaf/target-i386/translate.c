...
#include "helper.h"
#define GEN_HELPER 1
#include "helper.h" // Commented by Lei
...
//Added by Lei
// Bad idea, so comment it now
inline void invoke_block_start_cb_target_translate(CPUState * env, struct TranslationBlock *tb)
{
    //printf("invoke_block_start_cb_target_translate \n");
//	fflush(stdout);
	if(!tb)
	{
		printf("tb is null \n");
    ...
    //if((unsigned long)tb->pc > 1000000000) // user application
    //	return;
    if(DECAF_is_BlockBeginCallback_needed(tb->pc))
    {
      //LOK: While we can define a new TCG operation for tcg_gen_movi_ptr in tcg/tcg-op.h we will just use tcg_const_ptr macro instead, tcg_target_ulong is defined in tcg.h and

	//char info[512];
	//memset(info, 0x00, 512);
	//sprintf(info, "begincall translate, pc: %lu, size: %d\n", (unsigned long)tb->pc, tb->size);
	//printf("--- invoke all: pc: %lu, size: %d\n", (unsigned long)tb->pc, tb->size);
	//DECAF_log_info(info);

	//printf("--- invoke all: pc: %lu, size: %d, tb: %u\n", (unsigned long)tb->pc, tb->size, (tcg_target_ulong)tb);
	//fflush(stdout);
	//printf("--- invoke all: tb: %lu\n", (tcg_target_ulong)tb);
//#if TCG_TARGET_REG_BITS == 64

	//TCGv_i64 tmp = tcg_const_i64((tcg_target_ulong)tb);
      ...
      //TCGv_ptr tmpTb = tcg_const_ptr((tcg_target_ulong)tb);
//	printf("--- invoke all: tmpTB is %lu\n", (unsigned long)tmpTb);
//	fflush(stdout);
      //gen_helper_DECAF_invoke_block_begin_callback(cpu_env, tmpTb);
//	printf("--- invoke all: tmpTB 2 is %lu\n", (unsigned long)tmpTb);
//	fflush(stdout);
//       tcg_temp_free_ptr(tmpTb);
//	printf("--- invoke all: tmpTB 3 is %lu\n", (unsigned long)tmpTb);
//	fflush(stdout);
	#endif
	...
	{
		;//printf("False, impossible, pc is %lu \n", (unsigned long)tb->pc);
	}
}
//End 
...
// Added by Lei
extern target_ulong expected_cr3;
extern FILE * tcg_log_file;
extern target_ulong expected_pc_start;
extern target_ulong expected_pc_end;
// End add
                                                  ...
                                                  int search_pc)
{
	if(record_bb)
	{
		//printf("gen_intermediate_code_internal \n");
		fflush(stdout);
	}
    ...
    //if(((unsigned long)tb->pc >= 134512640) && ((unsigned long)tb->pc <= 134513700))
    if(0)
    {
	//printf("gen_code_internal, pc: %llu, cs_base: %lu, flags: %d, cr3: %llu\n", (unsigned long)tb->pc, (unsigned long)tb->cs_base, (int)tb->flags, (unsigned long)env->cr[3]);
	printf("gen_code_internal, pc: %llu, cs_base: %lu, flags: %d, cr3: %llu\n", tb->pc, (unsigned long)tb->cs_base, (int)tb->flags, env->cr[3]);
    }
    ...
    // Added by Lei
    // TODO: remove the callback here, since we have invoked callback in cpu_exec() for each block
#if 1
    if(DECAF_is_BlockBeginCallback_needed(tb->pc) )
    {
	...
	memset(info, 0x00, 512);
	sprintf(info, "begincall translate, pc: %lu, size: %d\n", (unsigned long)tb->pc, tb->size);
	//printf("-- gen_code_internal, pc: %lu, size: %d, tb: %u \n", (unsigned long)tb->pc, tb->size, (unsigned long)tb );
	//DECAF_log_info(info);
	...
	//	track_flag = 1;
      gen_helper_DECAF_invoke_block_begin_callback(cpu_env, tmpTb);
      //gen_helper_DECAF_invoke_block_begin_callback(cpu_env, tb);
	//printf("--- translate: tmpTB is %lu\n", (unsigned long)tmpTb);
	fflush(stdout);
      tcg_temp_free_ptr(tmpTb);
      //LOK: I wonder if I really have to call tcg_temp_free_ptr
      // since all of the other calls to tcg_const... don't have it
    ...
    // Added by Lei
    // dump tcg ops here
    target_ulong cur_cr3 = 0; // TODO: set cur_cr3 as global variable, and set upon tlb_callback?
    #if (defined(TARGET_I386) || defined(TARGET_X86_64))
        cur_cr3 = env->cr[3];   // for x86 only
    #endif
    
    // NOTE: try to avoid unnecessary operations, so we get expected information step by step
    // TODO: a global variable is better
    // expected_proc_cr3 = get_expected_cr3(); // get cr3 
    //printf("gen_code, cr3 is %lu", expected_proc_cr3);
    //printf("cr3: %lu, pc_start: %lu, pc_end: %lu \n", expected_cr3, expected_pc_start, expected_pc_end);
    if(!expected_cr3)
        goto continue_orig;


    //if(tcg_log_file)
    //    get_expected_pc_range(&expected_proc_pc_start, &expected_proc_pc_end); // get vma range of module

    // ignore TBs not in expected module
    // NOTE: if TBs of libraries are required, comment the code below.
    if(tb->pc > expected_pc_end ||  tb->pc < expected_pc_start)
        goto continue_orig;   

    if(((unsigned long)cur_cr3 == (unsigned long)expected_cr3))
    {
	//printf("get expected cr3 \n");
    	//log_file = fopen("/tmp/tcg_ops.log", "a+");
	if(tcg_log_file)
	{
	    //printf("------DUMP_START tb: %x, tb->PC: %lu, tb->cs_base: %lu \n", (unsigned long)tb, tb->pc, tb->cs_base);
	    fprintf(tcg_log_file, "------DUMP_START tb: %x, tb->PC: %lu, tb->cs_base: %lu \n", (unsigned long)tb, tb->pc, tb->cs_base);
            tcg_dump_ops(&tcg_ctx, tcg_log_file);
	    fprintf(tcg_log_file, "------DUMP_END \n\n");
 	    fflush(tcg_log_file);
        }
	//printf( "------DUMP_END \n\n");
    }
	...
	;
	//if(expected_proc_cr3 != 0) 
        //    printf("cur_cr3: %lu, expected_cr3: %lu \n", cur_cr3, expected_proc_cr3);
        //else
        //    printf("expected_cr3 is 0 \n");
    // End add
